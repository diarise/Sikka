...
queries_map = {
    ...
    "achats": (
        ...
        SELECT TOP 50 
            ...
            CAST(SUM(L.DL_MontantTTC) AS DECIMAL(18,2)) AS MONTANT_TTC,
            'FACTURE FOURNISSEUR' AS TypeDoc
        FROM F_DOCENTETE E WITH (NOLOCK)
        INNER JOIN F_DOCLIGNE L WITH (NOLOCK) ON E.DO_Piece = L.DO_Piece AND E.DO_Type = L.DO_Type
        LEFT JOIN F_COMPTET C WITH (NOLOCK) ON E.DO_Tiers = C.CT_Num
        WHERE E.DO_Type = 17
    ...
    "top_articles": (
        ...
        SELECT TOP 20 
            ...
            SUM(L.DL_Qte) AS QuantiteVendue, 
            SUM(L.DL_MontantTTC) AS MontantVendu
    ...
    "top_clients": (
        ...
        SELECT TOP 20 
            ...
            SUM(L.DL_MontantTTC) AS CA_Total, 
    ...
    "rotation": (
        ...
        SELECT TOP 20 
            ...
            SUM(L.DL_Qte) AS QuantiteVendue, 
            SUM(L.DL_MontantTTC) / NULLIF(SUM(L.DL_Qte), 0) AS PrixMoyen
        FROM F_DOCLIGNE L WITH (NOLOCK) 
        INNER JOIN F_DOCENTETE E WITH (NOLOCK) ON L.DO_Piece = E.DO_Piece AND L.DO_Type = E.DO_Type 
    ...
    "marge_brute": (
        ...
        SELECT TOP 20 
            L.AR_Ref AS CodeArticle, 
            MAX(ISNULL(A.AR_Design, L.DL_Design)) AS Libelle, 
            AVG(L.DL_PrixUnitaire) AS PrixVenteMoyen, 
            AVG(ISNULL(A.AR_PrixAch, 0)) AS PrixAchatMoyen,
    ...
    "dernieres_transac": (
        ...
        SELECT TOP 50 
            ...
            CASE E.DO_Type 
                WHEN 7 THEN 'Facture' 
                WHEN 17 THEN 'Achat' 
                WHEN 20 THEN 'Entrée Stock' 
                WHEN 21 THEN 'Sortie Stock' 
                ELSE CAST(E.DO_Type AS VARCHAR) 
            END AS Type
        FROM F_DOCENTETE E WITH (NOLOCK) 
        INNER JOIN F_DOCLIGNE L WITH (NOLOCK) ON E.DO_Piece = L.DO_Piece AND E.DO_Type = L.DO_Type 
    ...
    "solde_client": (
        ...
        SELECT 
            ...
            ISNULL(SUM(L.DL_MontantTTC), 0) AS TotalFacture, 
            ISNULL(SUM(R.RG_Montant), 0) AS TotalRegle,
